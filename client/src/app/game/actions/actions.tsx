...
const Actions = ({onClick, localGameState, mobile, id}: Props) => {
	...
	const Status = () => {
		...
		const followup =
			availableActions.includes('FOLLOW_UP') && availableActions.length === 1
		const opponentFollowup = availableActions.includes(
		...
		const turnMsg = turn
			...
			: "Opponent's Turn"
		const knockedOut =
			player?.board.activeRow === null && player.lives !== 3 && turn
		const changeHermit =
		...
		return (
			<>
				...
				<p>
					...
					{getPickProcessMessage(pickProcess)}
				</p>
			</>
		)
	}
